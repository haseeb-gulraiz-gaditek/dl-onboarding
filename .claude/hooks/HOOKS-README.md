...
> **Template note:** In `dl-onboarding`, hooks are **disabled by default** (`disableAllHooks: true` in `.claude/settings.json`) and the `sounds/` directory has been removed to keep the template lean. The hook scripts themselves are included as a reference implementation — if you want audio feedback during Claude Code sessions, see the "Installing Sounds" section below.
>
> To enable hooks in your venture:
> 1. In `.claude/settings.json`, set `"disableAllHooks": false`.
> 2. Populate `.claude/hooks/sounds/` (see section below or copy from [`disrupt-gt/course-materials`](https://github.com/disrupt-gt/course-materials/tree/main/.claude/hooks/sounds)).
> 3. Ensure `python3` is on `PATH`.
